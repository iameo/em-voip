...
def response_model(response=None, explicit_message=' ', allow_only_data=False):
    ...
    if response and allow_only_data:
        return jsonify(message=message or "fetched successfully", data=_encode_data(response), status=_status.HTTP_200_OK)

    data = response.get('data', {})
    status = response.get('status', _status.HTTP_200_OK)
    
    if not message:
        if status in range(200, 208):
            message = "success"
    ...
    return jsonify(data=_encode_data(data), message=f'{message}', status=status)
